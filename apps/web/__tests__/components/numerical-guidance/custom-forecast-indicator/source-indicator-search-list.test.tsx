...
describe('SourceIndicatorSearchList', () => {
  ...
  it('검색바에 지표 symnl 입력하면, 검색된 지표 리스트를 보여준다', async () => {
    // given
    render(
      <SWRProviderWithoutCache>
        <SourceIndicatorSearchList />
    ...
    // then
    await waitForElementToBeRemoved(() => screen.queryByText('MSFT(Microsoft Corporation)'));
    expect(await screen.findByText('AAPL(Apple Inc.)')).toBeInTheDocument();
  });
  ...
  it('지표를 클릭하면, 선택된 지표가 재료 지표로 추가된다', async () => {
    ...
    //when
    userEvent.click(await screen.findByText('MSFT(Microsoft Corporation)'));
